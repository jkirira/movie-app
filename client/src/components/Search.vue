<template>
    <div>
        ...
        <div class="flex flex-wrap justify-center lg:min-h-screen lg:p-10">
            ...
            <template v-else>
                <div class="flex justify-center p-10 mb-3 w-full">
...
export default {
    ...
    mounted() {
        let URL = "http://localhost:4000/api/v1/shows/search/" + this.$route.params.search_tag
        axios.get(URL)
              .then((response) => {
    ...
    methods: {
        search(){
            ...
            this.$router.replace({ name: 'movie_search', params: { 'search_tag': this.search_tag } })
        }
    }
}
