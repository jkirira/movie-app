<template>
<div>
    <!--search-->
    <div class="flex justify-center px-10">
        <div class="mb-3 w-full lg:w-7/10">
            <div class="input-group relative flex flex-wrap items-stretch mb-4">
                <input v-model="search_tag" type="search" class="form-control relative flex-auto min-w-0 block w-4/5 px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-green-500 focus:outline-none" placeholder="Search" aria-label="Search" aria-describedby="button-addon2">
                    <button @click="search" class="btn inline-block px-6 py-2.5 bg-green-500 max-w-1/5 my-2 lg:mx-4 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-green-500 hover:shadow-lg focus:bg-green-500  focus:shadow-lg focus:outline-none focus:ring-0 active:bg-green-500 active:shadow-lg transition duration-150 ease-in-out flex items-center" type="button" id="button-addon2">
                        <svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="search" class="w-4" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                            <path fill="currentColor" d="M505 442.7L405.3 343c-4.5-4.5-10.6-7-17-7H372c27.6-35.3 44-79.7 44-128C416 93.1 322.9 0 208 0S0 93.1 0 208s93.1 208 208 208c48.3 0 92.7-16.4 128-44v16.3c0 6.4 2.5 12.5 7 17l99.7 99.7c9.4 9.4 24.6 9.4 33.9 0l28.3-28.3c9.4-9.4 9.4-24.6.1-34zM208 336c-70.7 0-128-57.2-128-128 0-70.7 57.2-128 128-128 70.7 0 128 57.2 128 128 0 70.7-57.2 128-128 128z"></path>
                        </svg>
                    </button>
            </div>
        </div>
    </div>

        <div class="flex flex-wrap justify-center lg:min-h-screen lg:p-10">

            <div v-for="(movie, index) in movies" :key="index" class=" p-4 h-96 ">
                <movie-card :movie="movie" :next="'admin_movie_details'"></movie-card>
            </div>

        </div>
<!--    <script src="../../js/jquery.js"></script>-->
<!--    <script>-->
<!--    </script>-->

</div>
</template>

<script>
import MovieCard from "../MovieCard.vue";
import Vuetable from 'vuetable-2';
import VuetablePagination from 'vuetable-2';
import axios from "axios";
import $ from 'jquery'
export default {
    name: "Movies",
    components: { MovieCard,},
    data(){
        return{
            movies: '',
            search_tag: ''
        }
    },
    mounted() {
        let URL = "http://localhost:4000/api/v1/shows/"
        axios.get(URL)
              .then((response) => {
                  this.movies = response.data
                  console.log('Movies')
                  console.log(this.movies)
              }).catch((err) => {
                    console.log("error:", err)
                })
    },
    methods: {
        search(){
            if(!this.search_tag){
                alert('Cannot search empty values !')
                return;
            }
            // this.$router.push({ name: 'search_movie', params: { tag: this.search_tag } })
            let URL = "http://localhost:4000/api/v1/shows/search/" + this.search_tag
            axios.get(URL)
                  .then((response) => {
                      this.movies = response.data
                      console.log(this.movies)
                  }).catch((err) => {
                console.log("error:", err)
            })
        }
    }
}

</script>

<style>
.movie-grid{
    height: 500px;
    overflow: scroll;
}

@media screen and (min-width: 1024px){
    .movie-grid{
        height: auto;
        overflow: unset;
    }
}
</style>